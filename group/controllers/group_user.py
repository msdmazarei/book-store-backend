...
from repository.group_repo import validate_groups, validate_group, \
    check_group_title_exists, groups_by_presses, limit_groups_by_person
...
def delete(id, db_session, username):
    ...
    permit = has_permission_or_not(
        [Permissions.PERMISSION_GROUP_USER_DELETE_PREMIUM],
        permissions)
    if not permit:
        press_permit = has_permission_or_not(
...
def add_users_to_groups(data, db_session, username):
    ...
    if username not in ADMINISTRATORS:
        ...
        permit = has_permission_or_not(
            [Permissions.PERMISSION_GROUP_USER_ADD_PREMIUM],
            permissions)
        if not permit:
            press_permit = has_permission_or_not(
            ...
            if not (press_permit and is_user_group_owner(user.person_id,
                                                         group_entities)):
                logger.error(LogMsg.PERMISSION_DENIED,
...
def get_by_group(group_id, db_session, username):
    ...
    if username not in ADMINISTRATORS:
        ...
        if not permit:
            ...
            if not (press_permit and user_is_in_group(user.id, group_id,
                                                      db_session)):
                logger.error(LogMsg.PERMISSION_DENIED,
                             {'PERMISSION_GROUP_USER_GET': username})
                raise Http_error(403, Message.ACCESS_DENIED)
...
def add_group_by_users(data, db_session, username):
    ...
    group = add_group({'title': group_title, 'person_id': person_id},
                      db_session, username)
    del data['title']
...
def is_user_group_owner(person_id, groups):
    for item in groups:
        if person_id != item.person_id:
            logger.error(LogMsg.PERMISSION_DENIED,
                         {'group_press_is_not_person': person_id})
            raise Http_error(403, Message.ACCESS_DENIED)
    return True
