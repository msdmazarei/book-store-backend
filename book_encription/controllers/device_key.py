...
def get_user_devices(user_id, db_session, username):
    ...
    logger.debug(LogMsg.PERMISSION_CHECK,{username:Permissions.DEVICE_KEY_GET_PREMIUM})
    per_data = {}
    permissions, presses = get_user_permissions(username, db_session)
    if user_id == user.id:
        per_data.update({Permissions.IS_OWNER.value: True})
    ...
    result = db_session.query(DeviceCode).filter(DeviceCode.user_id==user_id).all()
    final_res = []
    for item in result:
